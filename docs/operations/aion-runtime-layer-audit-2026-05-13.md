...
| Perception | `PerceptionAgent` + `StructuredPerceptionAssessor` | event, recent memory, profile, optional OpenAI classifier | topic, intent, language, affective fallback, behavior feedback | partially verified; AI-assisted slice implemented | `tests/test_perception_assessor.py`; `tests/test_runtime_pipeline.py::test_runtime_pipeline_uses_ai_assisted_structured_perception_before_context`; PRJ-1196 focused packs | PRJ-1196 makes model-owned structured perception active when OpenAI is configured and keeps deterministic keyword hints as fallback. Full backend and production smoke are still pending for closure. |
...
| AI-assisted structured perception | implemented pending full validation | `ARCH-AI-PERCEPTION-001` / `PRJ-1196`: provider-backed structured perception now owns language/topic/intent when OpenAI is configured, with deterministic keywords only as validated fallback. |
