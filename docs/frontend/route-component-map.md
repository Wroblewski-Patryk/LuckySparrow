...
| Chat components | `web/src/components/chat.tsx` | `ChatFlowStage`, `ChatTranscriptMessageRow`, `ChatComposerShell` |
...
| Chat helpers | `renderChatMarkdown` in `web/src/lib/chat-markdown.tsx`; `transcriptMetadataSummary`, `chatDeliveryState`, `reconcileLocalTranscriptItems` in `web/src/lib/chat-transcript.ts`; `ChatFlowStage`, `ChatTranscriptMessageRow`, and `ChatComposerShell` in `web/src/components/chat.tsx` | `/chat` |
...
- `web/src/App.tsx` remains the route-rendering/component/state owner for most
  ...
  `web/src/components/app-icons.tsx`; chat flow stage, transcript row, and composer shell
