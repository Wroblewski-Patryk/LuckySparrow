...
- next improvement:
  ...
  - decide how far to push `aion_goal_milestone` before introducing richer milestone semantics such as due signals, milestone-level ownership, explicit dependency graphs, or explicit milestone objects with stronger lifecycle rules
  - decide whether `goal_milestone_arc`, `goal_milestone_pressure`, and `goal_milestone_dependency_state` should remain lightweight semantic compressions over milestone history and active tasks, or become part of a richer milestone-lifecycle model with stronger state transitions and due/deadline semantics
