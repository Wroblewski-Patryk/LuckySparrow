...
- current repo behavior can now also derive a lightweight `goal_progress_score` from active task status mix and recent completions, compare it to the previous reflected score to infer `goal_progress_trend`, persist short goal-level snapshots in `aion_goal_progress`, compress those histories into a semantic `goal_progress_arc`, maintain a lightweight `goal_milestone_state`, emit a lightweight `goal_milestone_transition` when progress crosses meaningful thresholds, sync lightweight `aion_goal_milestone` objects from that state, enrich those milestone objects with lightweight runtime signals like `goal_milestone_risk`, `goal_completion_criteria`, `goal_milestone_arc`, `goal_milestone_pressure`, `goal_milestone_dependency_state`, and `goal_milestone_due_state`, persist short `aion_goal_milestone_history` snapshots, and use those hints and objects in context, motivation, and planning
- next improvement:
  ...
  - decide how far to push `aion_goal_milestone` before introducing milestone-level ownership, explicit dependency graphs, due windows, or explicit milestone objects with stronger lifecycle rules
  - decide whether `goal_milestone_arc`, `goal_milestone_pressure`, `goal_milestone_dependency_state`, and `goal_milestone_due_state` should remain lightweight semantic compressions over milestone history and active tasks, or become part of a richer milestone-lifecycle model with stronger state transitions and due/deadline semantics
