...
- next improvement:
  ...
  - decide how far to push `aion_goal_milestone` before introducing richer milestone semantics such as dependencies, due signals, milestone-level ownership, or explicit milestone objects with stronger lifecycle rules
  - decide whether `goal_milestone_arc` and `goal_milestone_pressure` should remain lightweight semantic compressions over milestone history, or become part of a richer milestone-lifecycle model with stronger state transitions and due/deadline semantics
