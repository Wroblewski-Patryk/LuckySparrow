...
def _hash_password(password: str, *, salt: bytes | None = None) -> str:
    ...
    return f"pbkdf2_sha256${AUTH_PBKDF2_ITERATIONS}${actual_salt.hex()}${digest.hex()}"


def _verify_password(password: str, stored_hash: str) -> bool:
    try:
        algorithm, iteration_text, salt_hex, expected_hex = str(stored_hash).split("$", 3)
        ...
        return False
    actual = hashlib.pbkdf2_hmac("sha256", str(password).encode("utf-8"), salt, iterations).hex()
...
def _auth_cookie_name(settings) -> str:
    candidate = str(getattr(settings, "auth_session_cookie_name", AUTH_SESSION_COOKIE_DEFAULT) or "").strip()
    return candidate or AUTH_SESSION_COOKIE_DEFAULT


def _auth_session_ttl_hours(settings) -> int:
    return max(1, int(getattr(settings, "auth_session_ttl_hours", AUTH_SESSION_TTL_HOURS_DEFAULT)))


def _set_auth_cookie(*, response: Response, settings, session_token: str) -> None:
    response.set_cookie(
        key=_auth_cookie_name(settings),
        value=session_token,
        httponly=True,
        secure=app_environment(settings) == "production",
        samesite="lax",
        max_age=_auth_session_ttl_hours(settings) * 3600,
        path="/",
    )
...
def _app_settings_payload(*, profile: dict | None, preferences: dict | None) -> dict[str, Any]:
    profile = profile or {}
...
async def _require_app_auth(request: Request) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    session_token = request.cookies.get(_auth_cookie_name(settings))
    if not session_token:
        raise HTTPException(status_code=401, detail="Authentication required.")
    memory_repository = _memory_repository_from_request(request)
    session = await memory_repository.get_auth_session_by_token_hash(
    ...
    user = await memory_repository.get_auth_user_by_id(str(session.get("user_id", "")))
    if not isinstance(user, dict) or not bool(user.get("is_active", False)):
        raise HTTPException(status_code=401, detail="Authentication required.")
    await memory_repository.touch_auth_session(session_id=str(session["id"]))
    return user, session
...
async def app_register(
    ...
    memory_repository = _memory_repository_from_request(request)
    email = _normalize_auth_email(body.email)
    existing_user = await memory_repository.get_auth_user_by_email(email)
    if isinstance(existing_user, dict):
        raise HTTPException(status_code=409, detail="User with this email already exists.")

    user = await memory_repository.create_auth_user(
        user_id=_new_auth_user_id(),
        email=email,
        password_hash=_hash_password(body.password),
        display_name=body.display_name,
    )
    session_token = _new_auth_session_token()
    now = datetime.now(timezone.utc)
    await memory_repository.create_auth_session(
        session_id=_new_auth_session_id(),
        user_id=str(user["id"]),
        session_token_hash=_hash_session_token(session_token),
        expires_at=now + timedelta(hours=_auth_session_ttl_hours(_settings_from_request(request))),
        user_agent=request.headers.get("user-agent"),
        ip_address=request.client.host if request.client is not None else None,
    )
    await memory_repository.update_auth_user(
        user_id=str(user["id"]),
        last_login_at=now,
    )
    settings_payload = _app_settings_payload(
        profile=await memory_repository.get_user_profile(str(user["id"])),
        preferences=await memory_repository.get_user_runtime_preferences(str(user["id"])),
    )
    _set_auth_cookie(
        response=response,
        settings=_settings_from_request(request),
...
@router.post("/app/auth/login", response_model=AppMeResponse)
async def app_login(
    ...
    memory_repository = _memory_repository_from_request(request)
    email = _normalize_auth_email(body.email)
    user = await memory_repository.get_auth_user_by_email(email)
    if not isinstance(user, dict) or not _verify_password(body.password, str(user.get("password_hash", ""))):
...
async def app_logout(
    ...
    _, session = await _require_app_auth(request)
    await _memory_repository_from_request(request).revoke_auth_session(session_id=str(session["id"]))
    _clear_auth_cookie(response=response, settings=_settings_from_request(request))
    return {"ok": True}


@router.get("/app/me", response_model=AppMeResponse)
async def app_me(request: Request) -> dict[str, Any]:
...
@router.patch("/app/me/settings", response_model=AppSettingsResponse)
async def app_patch_me_settings(
    ...
    if body.preferred_language is not None:
        await memory_repository.set_user_profile_language(
            user_id=user_id,
    ...
    if body.proactive_opt_in is not None:
        await memory_repository.upsert_conclusion(
            ...
            content="true" if body.proactive_opt_in else "false",
            confidence=1.0,
            source="app_settings",
        )

    return _app_settings_payload(
...
async def app_chat_message(
    body: AppChatMessageRequest,
    request: Request,
) -> dict[str, Any]:
    user, _ = await _require_app_auth(request)
    event_response = await _handle_event_request(
        payload={
            "text": body.text,
