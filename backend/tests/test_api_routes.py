...
class FakeSettings:
    def __init__(
        ...
        *,
        ...
        auth_session_cookie_name: str = "aion_session",
        ...
        self.auth_session_ttl_hours = auth_session_ttl_hours
...
class FakeMemoryRepository:
        ...
        self.recent_memory = [
            {
                ...
                "summary": "Remembered preference update.",
    ...
    async def get_auth_user_by_id(self, user_id: str) -> dict | None:
        row = self.auth_users.get(user_id)
        return dict(row) if row is not None else None

    async def get_auth_user_by_email(self, email: str) -> dict | None:
        user_id = self.auth_users_by_email.get(str(email).strip().lower())
        if user_id is None:
            return None
        row = self.auth_users.get(user_id)
        return dict(row) if row is not None else None
    ...
    async def create_auth_user(
        ...
        *,
        ...
        self.auth_users[user_id] = row
        self.auth_users_by_email[row["email"]] = user_id
        return dict(row)
    ...
    async def upsert_conclusion(
        ...
        if kind == "response_style":
            self.user_preferences["response_style"] = content
        elif kind == "collaboration_preference":
...
def test_app_auth_register_sets_session_cookie_and_returns_user_snapshot() -> None:
    ...
    assert response.status_code == 200
    assert "aion_session=" in response.headers["set-cookie"]
    body = response.json()
...
def test_app_login_logout_and_me_roundtrip() -> None:
    client, _, _ = _client()
    ...
    assert "aion_session=" in login_response.headers["set-cookie"]
